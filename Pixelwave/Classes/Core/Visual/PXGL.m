...
/*
// A utility function. Reads the pixels from the currently bound buffer and
// inverts the output. Pretty ooptimized
//
// Assumes format = GL_RGBA
// Assumes type = GL_UNISGNED_BYTE
...
{	
	...
	// One row images can't be flipped
	if (height > 1)
	{
		// Loop through half of the image and swap each row with the row on the
		// opposite side of the image
		
		int halfHeight = height >> 1; // height / 2
		int bytesPerRow = width << 2;// width * 4
		
		GLubyte *curRowPtr, *oppRowPtr;
		GLubyte tempRow[bytesPerRow];
				
		curRowPtr = pixels; // First row
		oppRowPtr = pixels + (bytesPerRow * (height-1)); // Last row
		
		for (int i = 0; i<halfHeight; ++i)
		{
			// Swap row[i] with row[oppositeI]
			...
			// 1
			memcpy(tempRow, curRowPtr, bytesPerRow);
			// 2
			memcpy(curRowPtr, oppRowPtr, bytesPerRow);
			// 3
			memcpy(oppRowPtr, tempRow, bytesPerRow);
			
			curRowPtr += bytesPerRow;
			oppRowPtr -= bytesPerRow;
		}
	}
}
*/
