...
{
	PXParsedSoundData *soundData = calloc(1, sizeof(PXParsedSoundData));
	...
	if (soundData)
	{
		...
		soundData->milliseconds = milliseconds;

		soundData->byteCount = byteCount;

		if (soundData->byteCount > 0)
		{
			soundData->bytes = calloc(soundData->byteCount, sizeof(signed char));
		}
		else
		{
			soundData->bytes = NULL;
		}
	}
