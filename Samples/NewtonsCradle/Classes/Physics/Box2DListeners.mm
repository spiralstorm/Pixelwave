...
{
	// If there is no delegate, then no one is listening to the collision events
	if (!delegate)
	{
		B2_NOT_USED(contact);
		B2_NOT_USED(impulse);
	...
	// If their is no contact, then we are done.
	if (!contact)
	{
		return;
	}
	...
	// If one of the fixtures doesn't exist, then this isn't a valid collision.
	...
	// If one of the bodies doesn't exist, then this isn't a valid collision.
	if (!bodyA || !bodyB)
	{
		return;
	}

	// Grab the forces on A (0)
